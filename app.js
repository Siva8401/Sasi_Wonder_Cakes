      ...
      if (catProducts.length) {
        ...
        ${catProducts.map((p, idx) => {
          ...
          <div class="card" data-cat="${p.cat}">
            ...
            <button class="add-btn" onclick="addToCart(${globalIdx})" aria-label="Add ${p.name} to cart">
              Add to Cart
            </button>
          </div>
        `;
...
function renderProducts(filter = "all") {
  productGrid.innerHTML = PRODUCTS
    ...
    <div class="card" data-cat="${p.cat}">
      ...
      <button class="add-btn" onclick="addToCart(${idx})" aria-label="Add ${p.name} to cart">
        Add to Cart
      </button>
    </div>`
    )
